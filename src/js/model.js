...
const creatRecipeObject = function (data) {
  // const recipe = data.data.recipe // Same as below
...
export const loadSearchResults = async function (query) {
  try {
    ...
    state.search.page = 1; // Resets the page back to 1 for new search results.
  } catch (err) {
    console.error(`${err}💥💥💥💥`);
    throw err;
  }
...
export const updateServings = function (newServings) {
  state.recipe.ingredients.forEach(ing => {
    ...
    // newQt = oldQt * newSearvings / oldServings // 2 * 8 / 4 = 4
  });
